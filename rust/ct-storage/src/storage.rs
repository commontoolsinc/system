use crate::{Key, MappedKey, PlatformStorage, Result};
use async_stream::try_stream;
use futures_core::Stream;
use ranked_prolly_tree::{BincodeEncoder, Entry, MemoryStore, NodeStorage, Storage, Tree};
    ...
    S: Storage<Key>,
{
    ...
    pub fn open_memory() -> Result<CtStorage<NodeStorage<Key, BincodeEncoder, MemoryStore>>> {
        let storage = NodeStorage::new(BincodeEncoder::default(), MemoryStore::default());
...
async fn create_tree<S: Storage<Key>>(
    storage: S,
    root: Option<Vec<u8>>,
) -> Result<Tree<BRANCHING_FACTOR, S, Key>> {
    Ok(match root {
