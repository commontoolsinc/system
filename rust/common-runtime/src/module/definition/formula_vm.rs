...
/// definitions whose target is [Target::CommonFormulaVm], whose affinity
/// allows for local instantiation and whose body is [ModuleBody::SourceCode].
#[derive(NewType)]
#[new_type(skip(From))]
pub struct FormulaVmDefinition(ModuleDefinition);

impl FormulaVmDefinition {
...
impl TryFrom<ModuleDefinition> for FormulaVmDefinition {
    type Error = CommonRuntimeError;

    fn try_from(value: ModuleDefinition) -> Result<Self, Self::Error> {
        if Target::CommonFormulaVm != value.target {
            ...
            match value.affinity {
                Affinity::LocalOnly | Affinity::PrefersLocal | Affinity::PrefersRemote => {
                    match value.body {
                        ...
                        crate::ModuleBody::SourceCode(_) => Ok(FormulaVmDefinition(value)),
                    }
                }
...
mod tests {
    ...
    fn it_ensures_local_affinity() {
        ...
        FormulaVmDefinition::try_from(passing_module_definition).unwrap();
        ...
        assert!(FormulaVmDefinition::try_from(failing_module_definition).is_err());
    }
