...
//!
use crate::wasi::poll::Subscribe;
use crate::wasi::{HostInputStream, HostOutputStream, StreamError};
use anyhow::anyhow;
use bytes::Bytes;
use std::sync::{Arc, Mutex};
use tokio::sync::mpsc;

pub use crate::wasi::write_stream::AsyncWriteStream;
...
pub struct AsyncReadStream {
    ...
    _join_handle: crate::wasi::runtime::AbortOnDropJoinHandle<()>,
}
...
impl AsyncReadStream {
    ...
    pub fn new<T: tokio::io::AsyncRead + Send + Unpin + 'static>(mut reader: T) -> Self {
        ...
        let join_handle = crate::wasi::runtime::spawn(async move {
            loop {
                use tokio::io::AsyncReadExt;
                let mut buf = bytes::BytesMut::with_capacity(4096);
                let sent = match reader.read_buf(&mut buf).await {
                    Ok(0) => sender.send(Err(StreamError::Closed)).await,
